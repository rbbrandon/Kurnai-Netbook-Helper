/*[ Compilation unit ----------------------------------------------------------
   ...
   Description     : Implementation of CustomizedToolTip - http://www.codeproject.com/Articles/42050/ToolTip-With-Image-C
...
namespace KurnaiNetbookHelper
{
    ...
    internal class CustomizedToolTip : ToolTip
    {
        ...
        private const int TOOLTIP_HEIGHT = 120;
        ...
        void CustomizedToolTip_Draw(object sender, DrawToolTipEventArgs e)
        {
            try
            {
                ...
                myToolTipRectangle.Size = mySize;//e.Bounds.Size;
                ...
                BrightIdeasSoftware.OlvListViewHitTestInfo info = ((BrightIdeasSoftware.ObjectListView)parent).OlvHitTest(parent.PointToClient(Cursor.Position).X, parent.PointToClient(Cursor.Position).Y);
                Image toolTipImage = null;

                if (info != null && info.Item != null)
                    toolTipImage = ((LWTItem)info.Item.RowObject).Image;
                ...
                if (OwnerDraw)
                {
                    ...
                    else
                    {
                        ...
                        if (info != null && info.Item != null)
                        {
                            ...
                            // Get the scale of the x and y dimensions of the image, relative to the dimesion of the picture control (120x120).
                            int xScale = toolTipImage.Width / 120, yScale = toolTipImage.Height / 120;
                            int newWidth, newHeight;

                            // Scale the picture size down so it fits in the 120x120 box without losing its aspect ratio.
                            if (toolTipImage.Width > toolTipImage.Height)
                            {
                                newWidth = toolTipImage.Width / xScale;
                                newHeight = toolTipImage.Height / xScale;
                            }
                            else
                            {
                                newWidth = toolTipImage.Width / yScale;
                                newHeight = toolTipImage.Height / yScale;
                            }

                            mySize = new Size(newWidth, newHeight);
                            oldSize = mySize;
                        }

                        if (toolTipImage != null)
                        {
                            myInternalImageWidth = oldSize.Width;
                            //oldSize.Width += PADDING; //myInternalImageWidth +
                        }
                        else
                        {
